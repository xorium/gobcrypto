...
func TestCostValidationInHash(t *testing.T) {
	...
	if p.cost != hp.cost {
		t.Errorf(
			"newFromHash should maintain the cost at %d, but was %d",
			p.cost,
			hp.cost,
		)
	}
...
func TestGeneratingAndValidation(t *testing.T) {
	password := "pass123"
	cost := DefaultCost
	salt := "faf6132ae848fkjf"
	...
	if err := CompareHashAndPassword(passHash, []byte(password)); err != nil {
		t.Fatalf(
			"can't compare hash %s ans password %s: %v",
